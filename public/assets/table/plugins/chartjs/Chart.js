...
module.exports = function(Chart) {
	...
	var defaultConfig = {
		...
		scales: {
			xAxes: [{
				type: "linear", // scatter should not use a side axis
				position: "bottom",
			...
			if (options.scales) {
				items = items.concat(
					(options.scales.xAxes || []).map(function(xAxisOptions) {
						return { options: xAxisOptions, dtype: 'side' }; }),
		...
		helpers.each(extension, function(value, key) {
			if (key === 'xAxes' || key === 'yAxes') {
				...
				if (base.hasOwnProperty(key)) {
					helpers.each(value, function(valueObj, index) {
						var axisType = helpers.getValueOrDefault(valueObj.type, key === 'xAxes' ? 'side' : 'linear');
...
module.exports = function(Chart) {
	...
	var helpers = Chart.helpers;
	// Default config for a side scale
	var defaultConfig = {
		position: "bottom"
	};
	...
	Chart.scaleService.registerScaleType("side", DatasetScale, defaultConfig);
